...
from rclpy import ROSException
from ros2service import ROSServiceException
...
class JoyTeleop(Node):
    ...
    def register_action(self, name, command):
        ...
        try:
            ...
            self.al_clients[action_name] = rclpy.action.ActionClient(action_name, action_type)
    ...
    class AsyncServiceProxy(object):
        def __init__(self, name, service_class, persistent=True):
            while not self.name.wait_for_service(timeout_sec=2.0):
                if not rclpy.ok():
            ...
            self._service_proxy = rclpy.ServiceProxy(name, service_class, persistent)
    ...
    def get_action_type(self, action_name):
        try:
            return ros2topic._get_topic_type(rclpy.resolve_name(action_name) + "/goal")[
                0
            ][:-4]
        except TypeError:
    ...
    def get_service_type(self, service_name):
        if service_name not in self.service_types:
            try:
                self.service_types[service_name] = ros2service.get_service_class_by_name(
                    service_name
                )
